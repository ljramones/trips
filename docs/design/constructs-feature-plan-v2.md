...
5. ✅ Panel constructor now takes the three subtype services in addition to the registry; `WorldbuildingMenuController` wires them in. Registry remains read-only (no interface change).
...
### Phase E.1 — In-System Feature Foundation ✅ DONE (2026-05-30)
Establishes the structural foundation for the Phase E routing arc. Introduces the `TransitMode` taxonomy, extends `DriveType` to carry transit-mode sets, adds the `GateNetwork` entity (first persisted catalog entity outside the sealed hierarchies), extends `SolarSystemFeature` with `parentBodyId` + `catalogReferenceId` + `catalogReferenceKind` + `networkId` columns + `JUMP_POINT` featureType, implements deterministic per-star jump-point computation with Hill-sphere clean-zone rejection, wires the Spring-event-based activation hook, and extends the renderer to draw jump points. Full design at `docs/design/phase-e1-in-system-feature-foundation.md`.

1. ✅ **Step 1** — Read-only verification against the design's §10 list. 5 items confirmed; 9 divergences flagged (A: DriveType already typed enum with 25 values, not "simple enum"; B: AssetKind lacks TRANSPORT_NODE for §6.2 dispatch; C: no `parentBodyId` column on SolarSystemFeature; D: `FeatureType` is String-constants not enum; F: `findByParentBodyIdAndFeatureType` doesn't exist; G: Cataloged contract for GateNetwork; H: Hill-sphere + outer-boundary prior art in `accrete` package). All resolved before implementation.
2. ✅ **Step 2** — `TransitMode` enum (5 values: SUBLIGHT / JUMP_POINT / WORMHOLE / JUMP_GATE / WARP) + `DriveType` extended to carry `Set<TransitMode>` via constructor data (third arg after Category + DriveSpecs). Locked 25-value mapping: 22 SUBLIGHT (chemical/electric/nuclear/fusion/antimatter/sail/ramjet/exotic-non-FTL) + 1 JUMP_GATE (GRTUL_GATE per Caine Riordan canon, no onboard thrust) + 1 WARP (GALACTIC_HYPER per "strategic FTL transit") + 1 empty (NONE). `DriveTypeTest.partitionMatches25` exhaustively pins the partition as a regression guard. Suite at 3,629.
3. ✅ **Step 3** — `GateNetwork` pipeline. New top-level worldbuilding entity outside both sealed hierarchies; implements `Cataloged` but not `SpaceAsset` or `SpaceInfrastructure`. 8 new files: `GateNetworkLifecycle` enum (3 values: ACTIVE/DERELICT/REACTIVATED) + `GateNetwork` record (11 fields, `faction()→builderPolity` + `concealed()→false` per Divergence G) + `GateNetworkEntity` + `GateNetworkMapper` (no JSON LOBs — no collection fields) + `GateNetworkRepository` (4 finders) + `GateNetworkDesignerService` (with `findAllAsCataloged()` returning `List<Cataloged>` not `List<SpaceAsset>`) + `GateNetworkCatalogSeeder` (@EventListener, vacuous today — Catalog ships zero canonical GateNetwork constants in E.1). V13 migration creates the gate_network table. 58 new tests. Suite at 3,687.
4. ✅ **Step 4** — V14 migration + SolarSystemFeature schema extension + SpaceshipDesign.defaultAccessibleNetworkIds + CatalogedKind enum + JUMP_POINT featureType. V14 adds 4 new columns to solar_system_feature (parent_body_id, catalog_reference_id, catalog_reference_kind, network_id) + 3 indexes + default_accessible_network_ids_json CLOB on spaceship_design. `CatalogedKind` (5 values: SHIP, STATION, WEAPON_INSTALLATION, MEGASTRUCTURE, TRANSPORT_NODE) per Divergence B(β) — distinct from AssetKind (4 values), parallel-ordinal-stability for the 4 shared values. `JUMP_POINT` added as 16th String constant in `SolarSystemFeature.FeatureType` (not enum — per Divergence D). `SpaceshipDesign` record extended 19 → 20 fields with `defaultAccessibleNetworkIds: Set<String>`. New 19-arg compatibility constructor preserves all 11 existing caller sites unchanged. `findByParentBodyIdAndFeatureType` added to repository. 21 new tests. Suite at 3,708.
5. ✅ **Step 5** — `JumpPointComputer` with the §3.1 algorithm. Pure-function computer: takes `StarDisplayRecord` + `SolarSystemDescription`, returns `Optional<Point3D>`. Deterministic seed from `starId.hashCode()`, 100-iteration cap, rejection sampling against toroidal Hill-sphere exclusion zones. Static helpers: `hillSphereRadius(mass, sma, parentMass)` (unit-agnostic; verified against Earth=0.01 AU + Jupiter=0.35 AU), `computeOuterSystemBoundary` (max-planet-sma when planets present; `40 × cbrt(stellarMassSolar)` AU HYG-fallback otherwise), `sampleCandidate` (uniform spherical-shell sampling). 26 new tests across 7 sections including pathological iteration-cap exhaustion + position-in-band invariant + spherical-sampling smoke test. Suite at 3,734.
6. ✅ **Step 6** — `SolarSystemActivatedEvent` + `JumpPointActivationListener` + `SolarSystemSpacePane.render()` integration. Event published after `this.currentSystem = ...` at line 394, wrapped in try/catch matching existing event-publish patterns. Listener (@Component + @EventListener + @Transactional) iterates primary + companion stars; per-star: existsById short-circuit via `findByParentBodyIdAndFeatureType`, then computes via JumpPointComputer, then persists. Cartesian → spherical coordinate mapping for the entity's `orbitalRadiusAU`/`orbitalAngleDeg`/`orbitalHeightAU` columns. Multi-star feature naming (`"<starName> Jump Point"` vs plain `"Jump Point"`). Graceful failure at three levels. **First behavioral change in the running app for E.1**: activating a system triggers jump-point persistence. 16 new tests. CatalogSyncIntegrationTest from D.8 still green. Suite at 3,750.
7. ✅ **Step 7** — `SolarSystemRenderer` extended for JUMP_POINT visualization. Color = `Color.MEDIUMPURPLE`, size = 1.5× baseSize (smaller than JUMP_GATE's 2.0×, larger than RESEARCH_STATION's 1.0×), glow extended to cover JUMP_POINT (worldbuilding: gravitational anomaly with energy signature despite NATURAL category). `FeatureDescription.isPointType()` updated to recognise JUMP_POINT. Test-seam refactor: dispatch logic extracted to package-private statics `featureSizeMultiplier(String)` + `defaultFeatureColor(String)` so tests don't need full SolarSystemRenderer construction. 15 new tests. **Known trade-off**: first-activation-not-visible (description.features was built before the listener ran); second-entry shows jump points correctly. Suite at 3,765.
8. ✅ **Step 8** — Plan-doc entry (this section) + retroactive design doc at `docs/design/phase-e1-in-system-feature-foundation.md`. Architecture pins documented; Phase E.2 prerequisites confirmed met.

Architecture pins now in place:
- 5 transit modes (SUBLIGHT / JUMP_POINT / WORMHOLE / JUMP_GATE / WARP) via `TransitMode` enum
- `DriveType` carries `Set<TransitMode>` with locked 25-value mapping enforced by `partitionMatches25` test
- `GateNetwork` as first non-sealed-hierarchy persisted catalog entity; implements Cataloged but not SpaceAsset or SpaceInfrastructure
- `SolarSystemFeature` gains catalogReferenceId + catalogReferenceKind + networkId + parentBodyId columns; JUMP_POINT featureType
- `CatalogedKind` enum (5 values) as polymorphic-dispatch discriminator; distinct from AssetKind (4 values); parallel-ordinal-stability for shared values
- `SpaceshipDesign.defaultAccessibleNetworkIds` (Set<String>) — design-level transponder access
- `JumpPointComputer` with deterministic-seed algorithm + Hill-sphere exclusion + 100-iteration cap; self-contained Hill-sphere + outer-boundary utility math
- `SolarSystemActivatedEvent` + `JumpPointActivationListener` — first user-visible behavioral change in E.1
- `SolarSystemRenderer` draws JUMP_POINT as MEDIUMPURPLE sphere with glow
- Test-seam refactor: renderer dispatch extracted to package-private statics for testability without full instance construction

Net E.1 test delta: +147 tests (3,618 → 3,765).

### Phase E.2 — Catalog-reference population (next phase)
With E.1's structure in place, E.2 populates canonical instances:
1. Canonical `GateNetwork` entries (Aldenata Civilian, Aldenata Military, Posleen if applicable)
2. Place Troy as Megastructure-typed `SolarSystemFeature` in Sol
3. Place ISS as Station-typed `SolarSystemFeature` in Sol (Earth orbit)
4. Place SAPL elements as WeaponInstallation-typed `SolarSystemFeature`s across Sol's defense perimeter
5. Canonical wormhole mouths (paired Aldenata-network mouths in connected systems)
6. New FTL drive types (SHIFT_DRIVE, POSLEEN_INTERSTELLAR, ALDENATA_WORMHOLE_CAPABLE, etc.) deferred from E.1 §4.4
7. GRTUL_GATE deprecation path (legacy single-value drive retired in favour of GateNetwork-based modeling)

### Phase E.3 — Routing integration (after E.2)
With E.1's structure + E.2's data, routing extends to:
...
3. Drive-type-aware route computation: ship's `defaultAccessibleNetworkIds` ∩ candidate route's required network ids must be non-empty.
4. Route display labels gate segments distinct from warp transits.
...
### Phase F.2 — Aliases ✅ DONE (2026-06-02)

Universe-scoped fictional-name overlays on real astronomical targets (stars + exoplanets). Implements the §7.2 Aliases content category from the worldbuilding-platform-requirements doc. The first F-series phase to add universe-required (not universe-optional) content: every Alias must reference an active Universe row, and deleting a Universe cascades to its aliases. Full design + retroactive close-out at `docs/design/phase-f2-aliases.md`.

1. ✅ **Step 1** — Read-only audit + verification. 6 unknowns resolved across StarObject reach (~5 files), ExoPlanet reach (~3 files), Universe FK reach (already in place from F.1), StarPropertiesPane.fxml structure (rows 1-13 of Fictional Info tab, append-friendly), existing renderer tooltip behavior (lazy-installed "name::polity" — replaceable), FindStarInViewDialog autocomplete pattern (ControlsFX TextFields, in-view subset). D1 divergence: no ExoPlanet info panel exists; D1.a ratified — exoplanets surface aliases via tooltip ONLY. Design doc revised to reflect Step 1 findings + Step 2/3 combination per `ddl-auto=validate` discipline.
2. ✅ **Step 2** — `Alias` entity pipeline + V18 migration. 7 production files: `AliasTargetKind` enum (STAR / EXOPLANET — polymorphic discriminator + scalar id, mirroring SolarSystemFeature.catalogReferenceKind/Id pattern from E.1 Step 4) + `Alias` record (8 fields incl. universeId-required + sensible Cataloged defaults: name()=aliasText, source()=faction()="", concealed()=false) + `AliasEntity` (composite unique index on universe_id+target_kind+target_id; FK to universe(id) ON DELETE CASCADE — distinct from F.1's V16 SET NULL) + `AliasMapper` (bidirectional, no LOB) + `AliasRepository` (3 finders) + `AliasDesignerService` (find/save/delete + `findActiveAliasesForTarget` consulting UniverseFilteringService internally per §4.5 chokepoint discipline) + `UniverseFilteringService.getActiveUniverseIds()` extracted as public method. V18 migration: alias table with FK CASCADE, unique constraint, idx_alias_universe + idx_alias_target. Two-layer uniqueness enforcement: V18 unique constraint as source of truth + service-level pre-check for friendly `IllegalStateException` ("alias already exists for universe X + target Y/Z (existing: 'OtherName', id=...). Edit or delete the existing alias instead of creating a duplicate."). +55 tests. Suite at 4,035.
3. ✅ **Step 3** — Renderer tooltip integration (F1.b dynamic per-hover, format α). Replaces pre-F.2 lazy-installed "name::polity" cached tooltip with single-Tooltip-per-node + `setText()`-rebuilt-per-hover pattern. AliasTooltipFormatter extracted as pure JavaFX-free utility (13 tests run without Toolkit/Spring). Three tooltip surfaces formatted: interstellar star (name + polity + alias lines), solar-system star (name + spectral + distance + alias lines), planet (name + orbital metadata + alias lines). AliasDesignerService.findActiveAliasesForTooltip + AliasDisplay record + UniverseFilteringService.getActiveUniverseNamesById() pair so renderer makes one bulk fetch per hover, not one per alias. Mixed injection: StarRenderer takes service via constructor (back-compat 6-arg ctor preserved); BodyRenderer uses setter `setAliasService` mirroring existing setContextMenuHandler 4.1.6 pattern. Three short-circuit paths preserve zero-cost steady state (service null, targetId null, empty active set). +39 tests. Suite at 4,074.
4. ✅ **Step 4** — Fictional Info tab Aliases section (star-only per D1.a). StarPropertiesPane.fxml gains rows 14-16 (Separator + bold section header + multi-line wrap Label), append-friendly per F.1 §3.4 (no rowIndex shifts on existing rows 1-13). Controller adds 2 nullable ctor params (AliasDesignerService + UniverseFilteringService — zero RightPanelController changes since Spring DI propagates through); subscribes to broker via `filteringService.subscribeToFilterChanges(this::refreshAliasesSection)`; `populateAliasesSection()` called from `setStar()`; `clearData()` resets to placeholder. AliasTooltipFormatter extended with `formatAliasesAsBulletList(aliases, emptyPlaceholder)` — option (a) formatter reuse won; per-alias format identical to tooltip surfaces, only bullet prefix + emptyPlaceholder diverge. Four defensive null paths (aliasService null, star.getId() null, filteringService null, FXML pre-load timing). Separator FXML import omission caught by the FXML-loads test — structural-correctness tests prove their weight. +14 tests. Suite at 4,088.
5. ✅ **Step 5** — AliasesDialog + AliasEditorDialog + menu integration. Worldbuilding > Aliases... menu item (Alt+A mnemonic) opens a modeless BorderPane mirroring UniversesDialog (broker subscription + ignoreNextBrokerCallback self-suppression + dispose). Filter row: Universe ComboBox ("All active universes" default; lists all incl. inactive) + Target kind ComboBox (All / Star / Exoplanet). TableView columns: Universe / Kind / Target / Alias / Description excerpt. Create / Edit / Delete / Close buttons. AliasEditorDialog (single class, dual-mode via null-existing-Alias discriminator) handles both create + edit; ControlsFX TextFields autocomplete on target picker per Step 1's FindStarInViewDialog pattern; edit-mode locks universe + target kind radios + target combo via setDisable(true). OK-button ActionEvent filter idiom (`addEventFilter(ActionEvent.ACTION, evt -> { if (!attemptSave()) evt.consume(); })`) keeps dialog open on validation failure. Two-layer uniqueness UX flows service IllegalStateException through to dialog error label, dialog stays open for retry. +17 tests. Suite at 4,105.
6. ✅ **Step 6** — §9 acceptance gate (AliasFilteringInvariantsTest) + retroactive doc + plan rollup. @DataJpaTest with Flyway through V18 + Catalog seeders; sweeps R5.5 (universes don't leak — Legacy-only + Caine-only + multi-active scenarios) + R5.6 (real catalog unaffected by alias activity, across 4 activation combinations) + R5.7 (substantive for first time — alias appears with universe active, disappears when inactive; idempotent re-activation) + FK ON DELETE CASCADE (test universe with aliases deleted, aliases gone; Legacy aliases survive) + uniqueness constraint (duplicate (universe, target) throws IllegalStateException with friendly message; same text across different pairs allowed) + naming convention audit. R5.7 is substantive for the first time in F-series — F.1's was vacuous because no Alias rows existed at F.1 time. +11 tests. Suite at **4,116**.

Architecture pins now in place (F.2 specific):

- `Alias` as third Cataloged-but-not-SpaceAsset persisted entity (after GateNetwork E.1 + Universe F.1); record with 8 fields, Cataloged contract via sensible defaults
- `AliasTargetKind` enum with polymorphic discriminator + scalar id pattern (STAR / EXOPLANET); template for future polymorphic target references
- V18 alias table with FK CASCADE to universe(id) — distinct from F.1's V16 SET NULL; required-FK content cascades, optional-FK content nullifies
- Composite unique constraint on `(universe_id, target_kind, target_id)` enforces single-alias-per-target-per-universe; service-level pre-check surfaces friendly error
- Two parallel alias systems coexist: legacy `StarObject.aliasList` (Set<String> for catalog name variants, universe-agnostic) + F.2's universe-scoped `Alias` entity; F.2 does NOT touch the legacy system per §1.1
- `AliasTooltipFormatter` pure utility extracted for cross-surface formatting (renderer tooltips + Fictional Info tab); JavaFX-free → testable without Toolkit
- F1.b dynamic per-hover tooltip pattern (Tooltip.setText rebuild per mouse-enter); no cache flag, universe activation surfaces immediately
- Single dialog class for create + edit (null-entity discriminator + 4-field lock on edit); template for future F.x CRUD dialogs (F.3 Faction editor, F.4 Era editor)
- OK-button ActionEvent filter idiom for validation-aware dialogs
...
- **F.3 Factions** full structure: Faction entity extraction from `CatalogProvenance.sourceUniverse` String + `faction` String → Faction FK refactor (~10 files touch per F.1 Step 1's wider-reach finding) (R7.6.2)
- **F.4 Eras** per-universe + era visibility filtering (R7.7)
- **F.5 Visual presentation rules** per-universe color/icon overrides (R7.8)
- **F.6 Population rules** + SolarSystemFeature universe scoping (R7.10) — revisits the F.1 §3.4 transitive-visibility deferral
