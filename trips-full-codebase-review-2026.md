...
- **Status**: substantially done (Phase 7.9 + Bucket-A close-out). `PlotStarsCoordinatorTest` (12 tests) pins the pure helpers extracted from `MainSplitPaneManager.onPlotStarsEvent`. Bucket-A added two more test files under `service/importservices/`: `ImportResultTest` (9 tests across 4 `@Nested` classes — defaults, equality, mutability, toString) and `ImportTaskControlTest` (4 tests pinning the interface contract via an anonymous impl). Coverage areas now pinned: `ImportResult` (which a follow-up grep confirmed is **dead code** — no live callers in src/main; the test file flags it for future deletion/merge), `ImportTaskControl` interface shape. The full-Spring `CSVDataImportService` / `CHVDataImportService` integration paths still need TestFX coverage; `SolarSystemSpacePane.setSystemToDisplay` smoke test likewise remains.
...
| 7.9 | Add characterization + happy-path tests to `service/importservices/` (top priority). Then TestFX on the most-used dialogs. Then a smoke test for `SolarSystemSpacePane.setSystemToDisplay`. | 47 | **partial** — `PlotStarsCoordinatorTest` (12 tests) + `ImportResultTest` (9 tests) + `ImportTaskControlTest` (4 tests) + `RegCSVFileTest` (3 tests) cover the pure helpers and result-record contracts. Full-Spring TestFX coverage of `CSVDataImportService` / `CHVDataImportService` and a smoke test for `SolarSystemSpacePane.setSystemToDisplay()` still pending — needs TestFX harness scaffolding (no precedent in repo). |
