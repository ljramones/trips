...
- **Status**: pattern shipped (Bucket-A third pass). New `StarEditViewModel` POJO (~90 editable fields across overview/secondary/fictional/display tabs) + `StarEditMapper` static bidirectional converter with `toViewModel(entity)` and `applyToEntity(vm, target)`. `toViewModel` calls `Hibernate.initialize` on the alias collection inside the caller's transaction so the view-model is detached-safe; `applyToEntity` writes back to a freshly-loaded entity, replacing the alias `Set` in full to dodge LIE on the existing collection. New `StarEditMapperTest` (11 tests in 3 `@Nested` classes) pins both directions + the round-trip. Per-caller migration of the 7 existing `new StarEditDialog(starObject)` callsites still pending — that's surgical work touching each caller plus the dialog constructor itself, deferred so the tested DTO infrastructure can land cleanly first.
