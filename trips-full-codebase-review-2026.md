...
**Original Dominant Risk Areas — Remediated**:
1. **Bulk-import session hygiene & schema-migration story** — Flyway baseline/migrations are in place, production validates schema, and CSV import now uses a transactional bulk-load boundary plus flush/clear batching.
2. **FX-thread blocking on JPA queries** — solar-system jump/refresh runs through background `Task`s, stale loads are cancelled/ignored, and a TestFX smoke test now guards the async contract.
3. **Mutable global state** (`TripsContext`) — FX-thread invariants are documented and enforced defensively; high-risk mutators now warn if called off the FX thread.
4. **God-class growth** — the targeted renderer, workbench, service, controller, and procedural-dialog decompositions landed with focused collaborators and tests.
5. **Event-bus used as synchronous RPC** — the event graph was catalogued, dead events removed, and listener-publishes-follow-up cases were confirmed as broadcast/status fan-out rather than RPC misuse.
6. **Unit ambiguity on `StarObject.mass`** — stored masses are normalized to solar masses by migration and ingestion boundaries; the transfer-calculator heuristic is gone.

No critical runtime bugs or security vulnerabilities found in core paths. Test coverage is now substantially stronger in import services, controllers, and graphics panes; remaining work is release validation and optional product-polish benchmarking rather than blocker remediation.
...
- **Status**: done -- closed as a standalone item; `MainPane` system-tray startup was extracted and further decomposition is now tracked under Issue 18 (god classes). No separate work remains.
...
- **Status**: done -- repo-wide cleanup completed under Issue 41 (Phase 7.4): `scripts/check-logging.sh` reports 0 violations across the source tree and `maven-checkstyle-plugin` now fails the build on regressions.
...
- **Status**: done — Phase 3.1 documented the FX-thread-only invariant on the class Javadoc, switched `constellationMap` to `ConcurrentHashMap`, and wrapped both `@EventListener` methods on the class in defensive `FxThread.runOnFxThread`. Phase 7 hardening: high-risk mutators (`setDataSetContext`, `addDataSet`, `removeDataSet`) now call `assertFxThreadOrWarn(...)` which logs a warning naming the caller + offending thread if invoked off the FX thread, so future regressions are surfaced rather than corrupting state silently. The deeper "AtomicReference snapshots / typed mutator service" redesign was evaluated and explicitly judged not worth the churn — the current setup catches the actual bug class (cross-thread mutation) and the FX-thread convention is now enforced by code rather than just by convention.
...
- **Status**: done — `.gitignore` already excludes `*.csv` + `*.csv.zip` (with whitelist exceptions for the small `files/programdata/*.csv` data bundle and `tripsapplication/src/main/resources/**/*.csv`). The 878 MB `HYG-MERGED-2M-TRIPS-11012026202505.csv` is **not in git history** — it exists only in the working tree and was caught by `.gitignore` before any commit. Smaller files in history (`30ly.trips.csv`, `exoplanet.eu_catalog*.csv`, all <3 MB) are acceptable; a `git filter-repo` scrub would be high-risk for a marginal win. `scripts/verify-data-bundles.sh` (with `--list`, `--check`, `--update`) plus the tracked `scripts/data-bundles.sha256` manifest gives a fresh clone a single command to verify the downloaded bundles match what the maintainers have. CLAUDE.md "Where catalog data lives" section points at the script.
...
- **Status**: done (Phase 5.1 renames + closeout hoist). The three renames landed in Phase 5.1 (`solarsysmodelling` → `solarsystem.modelling`, `planetarymodelling` → `planetary.modelling`, `solarsystem/sol/` collapsed into `solarsystem/`). The closeout move addressed the bidirectional UI↔modelling coupling the issue called out: the existing `com.teamgannon.trips.model` package (which already held `AsteroidBelt`, `KuiperBelt`, `OortCloud`, `OrbitalParameters`, `Planet`, `StellarColor`, `SystemObject`) gained 5 more shared specs via `git mv` — `PlanetDescription`, `SolarSystemDescription`, `FeatureDescription`, `CometDescription` (all formerly in `planetary.modelling`), and `PlanetTypeEnum` (formerly in `solarsystem.modelling.accrete`). 33 consumer files had their imports rewritten. Before: `solarsystem/rendering/SolarSystemRenderer` imported `planetary.modelling.PlanetDescription` while `planetary/rendering/PlanetarySkyRenderer` imported `solarsystem.modelling.accrete.PlanetTypeEnum`; after: both depend on `model.*`, and `solarsystem/` no longer imports from `planetary.modelling` (and vice-versa). The residual `planetary.modelling.procedural → solarsystem.modelling.accrete.Planet` is modelling-to-modelling, not the UI↔modelling coupling the issue targeted. 2,767 tests pass.
...
- **Status**: done (Phase 4 decomposition + closeout `SolarSystemFactory` lifecycle). Phase 4 decomposition: 9 of 9 god classes shipped. Current line counts: `SolarSystemRenderer` 1,020 (was 2,004, -49%), `ProceduralPlanetViewerDialog` 1,508 (was 1,936, -22%), `WorkbenchEnrichmentService` 1,272 (was 1,832, -31%), `DataWorkbenchController` 1,581 (was 1,706, -7%), `JavaFxPlanetMeshConverter` 1,523 (was 1,644, -7%), `SolPlanetsInitializer` 1,354 (was 1,410, -4%), `PlanetarySkyRenderer` 889 (was 1,204, -26%), `SolarSystemService` 675 (was 793, -15%), `MainSplitPaneManager` 598 (was 670, -11%; coordinator extraction in Phase 7.14). The Sol-specific "unify with procedural via `ISolarSystemFactory`" suggestion landed in the closeout: new `service/factories/` package with `SolarSystemFactory` interface (`name`/`appliesTo`/`generate`), `SolarSystemFactoryResult` record (counts + identity), `SolarSystemFactoryRegistry` (priority-ordered dispatcher), and `ProceduralSolarSystemFactory` (`@Order(LOWEST_PRECEDENCE)` fallback wrapping `new StarSystem(...)` + `GeneratedSystemPersister.saveSystem`, with a guard against overwriting hand-curated "Direct" planets). `SolPlanetsInitializer` now implements the interface with `@Order(0)` and a `name`/`appliesTo`/`generate` triple; both its `@PostConstruct` hook and `SetContextDataSetEvent` listener route through `this.generate(sol)`. New tests: `SolarSystemFactoryRegistryTest` (9 tests, priority + fall-through + null-safety + result helpers) and `SolPlanetsInitializerFactoryContractTest` (4 tests, name + appliesTo case/whitespace/null). UI dialogs still call `SolarSystemService.saveGeneratedSystem` directly — the abstraction is in place for those migrations as follow-ups. 2,779 tests pass.
...
- **Status**: done. New `StarEditViewModel` POJO (~90 editable fields across overview/secondary/fictional/display tabs) + `StarEditMapper` static bidirectional converter with `toViewModel(entity)` and `applyToEntity(vm, target)`. `toViewModel` calls `Hibernate.initialize` on the alias collection inside the caller's transaction so the view-model is detached-safe; `applyToEntity` writes back to a freshly-loaded entity, replacing the alias `Set` in full to dodge LIE on the existing collection. `StarEditDialog` constructor now takes `StarEditViewModel`; `StarEditFormBinder` + `StarEditStatus` updated to carry the VM. All 7 callers (`StarPropertiesPane`, `StarContextMenuHandler`, `EditMenuController`, `ShowStarMatchesDialog`, `StarDisplayRecordCell`, `StarTablePane.editSelectedStar`, `StarTablePane.addNewStar`) now load the entity → map to VM → showAndWait → apply VM back to entity → updateStar. `StarEditMapperTest` (11 tests) pins both directions + the round-trip; `StarEditFormBinderTest` updated to seed VMs directly. 2,764 tests pass. The `Hibernate.isInitialized` defensive check is gone — the VM owns a plain `ArrayList<String>` for aliases, so there's no lazy collection to guard.
...
- **Status**: done at the typed-surface layer. `Page<...>` overloads added to the highest-risk finders: `StarObjectRepository` gains paginated variants of `findByDataSetNameOrderByDisplayName`, `findByDisplayNameContaining` (both dataset-scoped and global), `findByCommonNameContaining` (both variants), `findByConstellationName`, `findByConstellation`, `findByCatalogId` (both variants), `findBySolarSystemId`; `ExoPlanetRepository` gains paginated variants of `findByStarName` + `findBySolarSystemId`. Unbounded variants are intentionally **not** `@Deprecated` — bulk operations (transit calculation, route finding across the dataset, exports) legitimately need the full list. Per-caller migration of UI search dialogs is bucket-B work, not bucket-A: each migration requires a UX call (default page size, paging controls, total-count display) that's a per-dialog design decision, not a mechanical sweep.
...
- **Status**: done (Bucket-A re-audit). `InlineFieldValidation` helper provides `attachError` / `clearError` / `validate` API + CSS pseudo-class wiring; two worked-example migrations in place: `FindStarByCommonNameDialog` (single field) and `AddDataSetDialog` (6 required-field alerts → one shared status label + red-bordered fields). Audit of the remaining ~25 Alert-from-validation sites: **most are NOT field-validation candidates** — they're "select something from the list first" / "no datasets exist yet" / "more than 2 stars required for the route finder" precondition checks where there's no field to highlight, so a modal Alert is the correct tool. The one remaining true field-validation site (`StarEditDialog.changeClicked` catching `NumberFormatException` from `parseDoubleOrThrow`) doesn't know which field threw — a future per-field validation refactor inside that dialog could swap to inline, but it's a per-dialog design call rather than this issue's remit. Closed at "patterns shipped; remaining alerts are legitimately modal preconditions."
...
- **Status**: done (Phase 7 close-out, with signoff) — `V5__drop_unused_misc_columns.sql` drops 15 columns (`star_obj.misc_text1..5` + `misc_num1..5` + `solar_system.custom_data1..5`). Entity fields removed from `StarObject` + `SolarSystem`; CSV import (`AstroCSVStar`, `RegularStarCatalogCsvReader`) silently ignores incoming columns 45–54; 3 CSV exporters (`CSVDataSetDataExportTask`, `CSVQueryDataExportTask`, `StarTableExportService`) write empty/zero placeholders for those slots so downstream consumers parsing by index stay stable; `StarEditFormBinder` lost the user-tab text-field plumbing; `StarEditDialog.fxml` "User Special Info" tab replaced with a slim "Display" tab containing only the `forceLabel` CheckBox. `StarEditFormBinderTest` updated to match the new signature. Test count went from 2,741 → 2,740 (lost the `shouldPopulateMiscTextFields` test along with the feature). Path forward if extensibility is wanted later: one JSON column + JSON Schema validator per the original suggestion.
...
- **Status**: done (Phase 6.7 + Bucket-A sweep). The original 9 detail-panel + maneuver-table strings externalised in 6.7. Bucket-A finished the sweep across the rest of `spaceshipmodeller/ui/*`: 4 remaining placeholder strings in `SpaceshipDesignerPanel` and `SpaceshipEditorDialog` now use `designer.table.placeholder`, `designer.validation.empty`, `editor.carriedTable.placeholder`, `editor.messages.empty` keys. Grep confirms zero remaining hardcoded `new Label("...")` / `new Button("...")` strings under `spaceshipmodeller/ui/`. The build-time CI scan to reject regressions is the optional follow-up.
...
- **Status**: done (pattern + helper + 9 TableView migrations). The original Phase 6.4 worked example (`TransferPlannerPanel.configureNodeTable`, percentage bindings + Vgrow) was extended by the closeout: new `com.teamgannon.trips.javafxsupport.ResponsiveLayouts` helper (`bindColumnPercentages` / `fillVerticalSpace` / `fillHorizontalSpace`) with 6-test contract, 9 additional TableViews migrated (`NebulaListDialog`, `NebulaCatalogImportDialog`, `SpaceshipEditorDialog`, `TransferPlanDialog`, `SpaceshipDesignerPanel`, `DisplayAutoRoutesDialog`, `RoutingPanel`, `DataSetManagerDialog`, `StarTableColumnFactory`, `WorkbenchPreviewManager`) using one of three idioms: `CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN` (where the last column should absorb leftover space), `CONSTRAINED_RESIZE_POLICY_ALL_COLUMNS` (proportional redistribute for many-column data tables), or explicit percentage bindings via the helper. `CLAUDE.md > Responsive Table & Layout Defaults` documents all three idioms with worked-example pointers and flags the per-dialog visual validation at 1024×768 / 4K as the explicit follow-up eye-check. 2,785 tests pass.
...
- **Status**: done. promptText + Tooltip pairs now on ~45 text inputs across 18 dialogs: the 2 from the Phase 7 close-out (FindStarByCommonNameDialog, FindStarByCatalogIdDialog), 11 added in the first Bucket-A pass (search dialogs, dataset pickers, exoplanet/gaia import dialogs, routing dialogs, transit filter pane, query dialog), and 5 added in the finisher pass: `RotationDialog` (XYZ angle fields), `StarNotesDialog` (notes TextArea), `ScriptDialog` (program + output TextAreas), `RADecToXYZDialog` (RA/Dec/distance), `EquatorialToGalacticCoordsDialog` (6 coord fields). Common hints: catalog IDs ("e.g. HIP 71683, HD 209458"), common names ("e.g. Sol, Alpha C, Vega"), units ("degrees", "light-years"). Every typed-input dialog under `dialogs/*`, `controller/*`, `graphics/*`, `scripting/*` is covered.
...
- **Status**: done. Phase 7.1 narrowed the catch to `IllegalArgumentException | ArrayIndexOutOfBoundsException | NullPointerException` (covers `NumberFormatException` via its IAE superclass). The "summary of first N bad rows" UX now ships too: `RegCSVFile.recordBadRow(snippet)` captures up to `MAX_BAD_ROW_SAMPLES` (10) entries of `"row N: ExceptionType: message"`, `BulkLoadService.loadCsvDataset` returns a `LoadOutcome` carrying reject count + samples, and `CSVLoadTask` threads them into `FileProcessResult.sampleBadRows` + the post-import status message and `log.warn` summary. New `RegCSVFileTest` pins the cap behaviour (rejects past the cap still counted, sample list is unmodifiable). 2,767 tests pass.
...
- **Status**: done (Phase 7.3 + Bucket-A audit). The two call-out sites fixed in 7.3 (`MainPane` icon load, `ScriptDialog.loadScriptFile`). Bucket-A grep across `service/importservices/` for `new (Buffered|File)(Reader|InputStream|Writer|OutputStream)` returned zero hits outside try-with-resources — that package was already clean. No further sweep needed.
...
- **Status**: done -- investigated, **not viable for this codebase's architecture** (the L2 cache from Issue 53 is the chosen alternative). Phase 7.8 added `@Basic(fetch = LAZY)` annotations to 10 `@Lob` fields (DataSetDescriptor ×6, ExoPlanet ×3, StarCatalogIds ×1). Phase 7 close-out wired the Hibernate enhancement plugin (`org.hibernate.orm:hibernate-maven-plugin:7.2.1.Final` — the old `org.hibernate.orm.tooling:hibernate-enhance-maven-plugin` was renamed during the Hibernate 7 ASL2 relicense, see [discourse thread](https://discourse.hibernate.org/t/version-7-0-cannot-find-the-hibernate-enhance-maven-plugin-in-maven/11547)). 2,740 tests still passed — but on actual app startup, `DataSetDescriptor.getRoutes()` (called by `PlotManager.drawAstrographicData` on the FX thread, outside any session) threw `LazyInitializationException`. Audit found ~30 cross-tx readers of the marked LAZY fields (PlotManager + DisplayScoreCalculator + WorkbenchEnrichmentService + StarTableExportService + several Gaia/edit dialogs + ProceduralPlanetPersistenceHelper). All LAZY annotations + the enhancement plugin reverted in close-out. **The L2 cache (Issue 53) is the real win for this codebase** — same "don't re-hit the DB" goal, doesn't require detached-entity hygiene. Re-enabling LAZY would need a sweep wrapping each LOB-reading UI/export path in a service-layer `@Transactional` boundary — a separate, sprint-scale architectural change.
...
- **Status**: done. `PlotStarsCoordinatorTest` (12 tests) pins the pure helpers extracted from `MainSplitPaneManager.onPlotStarsEvent`. Bucket-A added `ImportResultTest` (9 tests across 4 `@Nested` classes — defaults, equality, mutability, toString) and `ImportTaskControlTest` (4 tests pinning the interface contract via an anonymous impl); `ImportResult` is dead code in src/main, flagged in the test for future deletion/merge. Closeout adds `CSVDataImportServiceTest` (7 tests) and `CHVDataImportServiceTest` (7 tests, reuses CSV's recording fixtures) that drive each `Service<FileProcessResult>` through real `Task` lifecycle (succeed / null-result-on-succeed / fail / cancel) and pin: the StatusUpdate + AddDataSet + SetContextDataSet event fan-out, the `ImportTaskComplete` callback, and `processDataSet`'s property bindings (label + progress bar + cancel-button-disable). `RegCSVFileTest` (3 tests, Issue 38) pins the bad-row sample collector. Follow-up hardening added `SolarSystemSpacePaneAsyncLoadTest`, which uses TestFX to prove `setSystemToDisplay()` returns before the service-backed load completes and renders only after the background `Task` succeeds; that test also caught and removed a dead private `root` parent that made the pane non-embeddable in a normal scene graph. Focused Java 25 test passed.
...
- **Status**: done. Accessibility annotations now on **35 dialogs** (~280 individual calls): the 4 from Phase 7.11, 11 from the first Bucket-A round, 13 from the second Bucket-A round (`ViewPreferencesDialog`, `ExportQueryDialog`, `UpdateStarObjectWithRecordDialog`, `Load10ParsecStarsDialog`, `FindAllByConstellationDialog`, `FindStarInViewDialog`, `ShowStarMatchesDialog`, `SesameNameResolverDialog`, `RouteDialog`, `DataSetDescribeDialog`, `ExportDialog`, `ComputerInventoryDialog`, `CompareDBDialog`), 9 from the finisher pass (`AboutDialog`, `ExamplesDialog`, `EachTimeStartDialog`, `RotationDialog`, `StarNotesDialog`, `ScriptDialog`, `RADecToXYZDialog`, `EquatorialToGalacticCoordsDialog`, `NebulaListDialog`), plus the two large composite forms originally flagged for a focused pass: `AddPlanetDialog` and `SpaceshipEditorDialog`. Both now annotate dialog panes, primary controls, validation/status surfaces, and action buttons.
...
- **Status**: done (Phase 7.10 + close-out + 4 Bucket-A passes + polish pass). theme.css carries the CSS-variable palette + 25 utility classes: the original 7 (.trips-bold, .trips-text-sm/md, .trips-text-bold-md, .trips-text-muted-sm, .trips-text-muted, .trips-text-section-header, .trips-panel-mid), 5 second-pass classes (.trips-text-bold-md-12, .trips-text-bold-lg, .trips-text-bold-sm, .trips-text-italic-muted-sm, .trips-text-italic-warn), and 13 closeout classes (.trips-text-form-label, .trips-text-form-label-sm, .trips-text-form-info, .trips-text-hz-optimal, .trips-text-hz-max, .trips-text-cooling, .trips-text-warming, .trips-button-action-success, .trips-badge-true, .trips-badge-false, .trips-badge-hz-optimal, .trips-badge-hz-extended, .trips-badge-hz-outside). Stylesheet loaded on the primary Scene by PrimaryStageInitializer, and `DialogUtils.applyTheme(Dialog<?>)` now gives standalone programmatic dialogs the same stylesheet without duplicating classpath lookup code. Across all migration passes: ~84 callsites converted to `getStyleClass().add("trips-…")` across 22 files; the closeout pass alone replaced 47 inline `setStyle(...)` strings in `ProceduralPlanetViewerDialog` (29), `PlanetTab` (13), and `PlanetDialog` (5). The remaining inline styles are dynamic per-instance state, genuinely bespoke per-tab colours, or isolated legacy callsites; no blocker theme work remains.
...
- **Status**: done — landed with Issue 31's V5 migration. The "User Special Info" tab is now a slim "Display" tab containing only the `forceLabel` CheckBox (the one surviving control). The 10 raw `miscN`/`miscNumN` text fields are gone.
...
| 4.2 | `ProceduralPlanetViewerDialog` **1,936 → 1,314 lines** (-32%). Nine focused helpers extracted: `PlanetScreenshotExporter` (PNG capture), `PlanetCameraController` (mouse + scroll + keyboard + auto-spin Timeline), `PlanetTerrainClassifier` (pure-function classifier + isIcyWorld predicate), `PlanetAtmosphereRenderer` (atmosphere shell with terrain-aware sizing/colour), `PlanetPoleMarker` (N/S marker spheres), `PlanetLegendSection` (stateless TitledPane builder), `PlanetClimateZoneOverlay` (tropic + polar latitude rings), and the closeout pair: `RiverNetworkRenderer` (164 lines — river chain rendering with flow-accumulation width scaling and rainfall-driven fallback) and `PlateBoundaryRenderer` (148 lines — colour-coded convergent/divergent/transform/inactive boundaries). The remaining big chunks (`renderPlanet`, the 7 createXSection side-panel builders, `regeneratePlanet` + GenerationProgressListener) need the bigger ViewModel-split design pass to come out clean — they read/write multiple dialog fields and listener wiring directly — and stay deferred at the dialog's current ~1,300 lines. Full non-integration suite 2,785 tests green. | 18 | **done** |
| 4.3 | `WorkbenchEnrichmentService` **1,832 → 989 lines** (-46%). Five stateless helper classes extracted, all in the same package: original Phase 4.3 trio `StellarEstimators` (396 lines — 12 pure-function photometric / spectral estimators), `TapHttpClient` (108 lines — TAP `/sync` POST with retry-on-429/5xx + shared `HttpClient`), `TapCsvParser` (83 lines — CSV splitting / unquoting / header lookup / safe double parsing); plus the closeout pair `CatalogIdExtractor` (193 lines — TYC-preferred SIMBAD ID extraction, longest-digit-sequence Gaia source_id parsing, HIP / numeric ID helpers, normalize / escape utilities, dedup append-token helper, 3-tier `getPreferredSimbadName` fallback) and `GaiaStellarParamsClient` (212 lines — `StellarParams` record + `fetchByGaiaIds` ADQL submit-and-parse + `applyTo(star, params)` missing-field-only updater with provenance tags). Service is now a thin orchestrator on top of these. The per-catalogue ID-extractor utility that the original status flagged as the prerequisite for the bigger "EnrichmentSource interface" split is now in place; that interface itself remains as an optional future step. Two dead rate-limit counter fields (`debugSkipCount`, `distanceRejectCount`) dropped — their gated `log.info` are now `log.debug` inside `StellarEstimators`. New `CatalogIdExtractorTest` (21 tests across 7 `@Nested` classes) pins the extractor contract; existing `WorkbenchEnrichmentServiceTest` keeps asserting against `TapHttpClient.HTTP_CLIENT`. 2,806 tests pass. | 18 | **done** |
| 4.4 | `DataWorkbenchController` **1,706 → 1,303 lines** (-24%). Three helpers extracted: `WorkbenchPreviewManager` (219 lines — preview table + pagination + page-lazy CSV loading + table-rebuild), `WorkbenchMappingPersistence` (91 lines — load/save the two-column source→target field mapping CSV plus the `normalizeFieldName` fuzzy-match helper), and the closeout `WorkbenchExoplanetTab` (376 lines, same package) which realises the context-object pattern: a `Bindings` record bundles the 20 @FXML controls (preview / match tables, 13 columns, file-status / match-stats labels, log area, progress bar, skip-duplicates checkbox); the controller hands them over once via `bind(...)` and delegates each `@FXML` on-action to a one-line call into `onLoadCsv` / `onMatch` / `onImport` / `onClear`. The tab class owns the `parsedExoplanets` / `matchResult` state, the observable preview + match row lists, the 4 FX-thread-safe status helpers, and the Task spawning. The remaining enrichment handlers (~400 lines across `onEnrichDistances` / `onEnrichMissingDistancesLive` / `onPhotometricEstimation` / `onEnrichMasses` / `onEstimateMassPhotometric` / `onEstimateTemperature` / `onEstimateSpectral` / `onCrossFillTempSpectral`) follow the same shape but read controller-scoped `@FXML` fields (`liveTapBatchField`, `liveTapBackoffField`, `photo*` / `spectral*` / `temp*` fields), so a similar extraction would need a wider bindings bundle — a second-pass cut, not part of this closeout. 2,806 tests pass. | 18 | **done** |
| 4.5 | `JavaFxPlanetMeshConverter` **1,644 → 1,246 lines** (-24%). Three focused helpers extracted in the same package: `PlanetColorPalette` (166 lines, mined first — six 9-stop colour arrays + lookup / interpolation methods); the closeout pair `MeshVertexAveraging` (249 lines — `VertexData` record + `buildVertexData` quantized-position-hash deduplication + four `computeAveragedHeights*` variants + `calculateAverageHeight` + `toFloatArray` / `toIntArray` helpers) and `TerrainNormals` (84 lines — fan-triangulation face-normal accumulation with renormalisation + radial-fallback). Converter keeps the public `getColorForX` / `createMaterialForX` / `computeTerrainNormals` API intact via static imports; `TerrainType` stays on the converter (3 other files import it). The remaining bulk is the convert* mesh-build pipeline (~9 overloads) and the material-creation methods — JavaFX-mesh-specific code that belongs on the converter. 2,785 tests pass. | 18 | **done** |
| 4.6 | `SolPlanetsInitializer` **1,410 → 1,354 lines** (-4%). `SolFeaturesFactory` (100 lines) extracted: builds the Main Asteroid Belt (2.1-3.3 AU) and Kuiper Belt (30-50 AU) as `SolarSystemFeature`s, with the existing idempotency check (skip if features already exist for the system). The "Sol as magical special case" concern from Issue 18 was the actual architectural worry and is now resolved by the closeout `SolarSystemFactory` work: `SolPlanetsInitializer implements SolarSystemFactory` (`@Order(0)`), and the `@PostConstruct` + `SetContextDataSetEvent` hooks now route through `this.generate(sol)` instead of bespoke startup logic. The bulk of the remaining lines is hand-curated astronomical data — splitting that doesn't add architectural value. | 18 | **done** |
| 4.7 | Three remaining big classes from the original Phase 4 list. **SolarSystemService 793 → 669 lines (-16%)**: `GeneratedSystemPersister` (227 lines) extracted as a new `@Component` owning the three ACCRETE save paths — `savePlanets` (planets + moons + simulated-row cleanup + planet-count + habitable-zone update), `saveBelts` (asteroid + Kuiper `SolarSystemFeature` rows), and the `saveSystem` convenience that chains both. Cross-service dependency on `findOrCreateSolarSystem` is passed in as a `Function<StarObject, SolarSystem>` so the persister stays decoupled. `SolarSystemService` keeps the public `saveGenerated*` methods as thin delegates so existing callers (`StarContextMenuHandler.java:303`) don't change. Two stale `{:.2f}` log placeholders fixed in passing (SLF4J doesn't grok Python-style format spec). **PlanetarySkyRenderer 1,204 → 889 lines (-26%)**: two cuts. `MilkyWayBackdrop` (197 lines) — the entire procedural Milky Way subsystem (own seeded `Random`, own `Group`, brightness-tier materials palette, and the galactic→equatorial transform that's used nowhere else in the codebase). `PlanetSkyMath` (175 lines) — pure-function helpers: `solveKeplerEquation`, `calculatePlanetOrbitalPosition` (Keplerian propagation with full ω/i/Ω rotation), `calculatePlanetApparentMagnitude` (type-keyed base + 5·log₁₀(d)), `adjustMagnitude` (catalogue→observer rescale), `getPlanetColor`. Renderer keeps a `MilkyWayBackdrop` field and delegates render/clear; `AU_TO_LY` now sourced from the helper. `MainSplitPaneManager` **670 → 598 lines (-11%)** via Phase 7.14's `PlotStarsCoordinator` extraction (covered in row 7.14 below): the inlined orchestration logic of `onPlotStarsEvent` (~50 lines + two geometry helpers) moved to a dedicated `@Component` with a 12-test unit-test suite. The other 3 listeners were re-audited and confirmed as already-thin dispatchers — extracting them would create a large dependency surface for modest payoff. Full non-integration suite 2,779 tests green throughout. | 9 (followup), 18 | **done** |

## Phase 5 — Package Consolidation

| # | Action | Issue | Rough effort |
|---|---|---|---|
| 5.1 | Three of the four renames applied in a single commit: `solarsysmodelling` → `solarsystem.modelling` (23 files), `planetarymodelling` → `planetary.modelling` (32 files), `solarsystem/sol/` collapsed into `solarsystem/` (3 files). 58 files moved, 134 import-site rewrites, package decls updated in every moved file, test tree mirrored. The closeout move addressed the bidirectional UI↔modelling coupling: `com.teamgannon.trips.model` (which already held `AsteroidBelt`, `KuiperBelt`, `OortCloud`, `OrbitalParameters`, `Planet`, `StellarColor`, `SystemObject`) gained 5 more shared specs via `git mv` — `PlanetDescription`, `SolarSystemDescription`, `FeatureDescription`, `CometDescription`, `PlanetTypeEnum`. 33 consumer files had imports rewritten. `solarsystem/` no longer imports from `planetary.modelling` (and vice-versa); residual `planetary.modelling.procedural → solarsystem.modelling.accrete.Planet` is modelling-to-modelling, not UI-to-modelling. Full non-integration suite 2,767 tests green. | 17 | **done** |

## Phase 6 — Medium Bugs & UX

| # | Action | Issue(s) | Rough effort |
|---|---|---|---|
| 6.1 | Stop entities escaping into dialogs. Introduce DTOs for edit flows (`StarEditViewModel` first). Load with `JOIN FETCH` in services. | 23 | **done** — new `StarEditViewModel` (~90 fields) + `StarEditMapper` (`toViewModel` / `applyToEntity`); `Hibernate.initialize` called inside the caller's tx so the VM is detached-safe. `StarEditDialog`, `StarEditFormBinder`, `StarEditStatus` all now carry the VM. All 7 callers migrated (`StarPropertiesPane`, `StarContextMenuHandler`, `EditMenuController`, `ShowStarMatchesDialog`, `StarDisplayRecordCell`, `StarTablePane.editSelectedStar`, `StarTablePane.addNewStar`). `StarEditMapperTest` (11 tests) + `StarEditFormBinderTest` (refactored to seed VMs). |
| 6.2 | Add `Pageable` overloads to every unbounded `List`-returning repo method. Deprecate the unbounded variants. Migrate callers. | 24 | **done** at the typed-surface layer — `Page<...>` overloads on the highest-risk finders in `StarObjectRepository` + `ExoPlanetRepository`. Unbounded variants intentionally not `@Deprecated` (bulk ops legitimately need full lists). Per-UI-caller migration is per-dialog UX work (default page size, paging controls), not mechanical sweep. |
| 6.3 | Add `@EntityGraph` variants for ExoPlanet ↔ hostStar joins; remove N+1s in the planet rendering path. | 25 | **done** — investigated, no actual N+1 in the current code path: `hostStarId` is a raw String FK with no `@ManyToOne`, no caller does loop-and-fetch. Javadoc clarification added to `findByHostStarId`. |
...
| 6.6 | Remove unused `miscN` / `customDataN` columns; if extensibility is wanted, replace with a single JSON column + schema validator. | 31, 54 | **done** (with user signoff) — `V5__drop_unused_misc_columns.sql` drops 15 columns (`star_obj.misc_text1..5` + `misc_num1..5` + `solar_system.custom_data1..5`); entity fields removed; CSV importers silently ignore those slots; 3 CSV exporters write placeholder values to keep column indices stable; `StarEditDialog.fxml` "User Special Info" tab replaced with a slim "Display" tab containing only the `forceLabel` checkbox. JSON-column path remains the future option if extensibility is wanted again. |
| 6.7 | Externalize remaining hardcoded strings in `spaceshipmodeller/ui/*` to the existing `.properties` bundle. Add a CI scan rejecting hardcoded strings under that package. | 32 | **done** — original 9 detail-panel / maneuver-table strings externalised in Phase 6.7; Bucket-A sweep finished the rest of `spaceshipmodeller/ui/*` (4 more keys: `designer.table.placeholder`, `designer.validation.empty`, `editor.carriedTable.placeholder`, `editor.messages.empty`). Grep confirms zero hardcoded `new Label("…")` / `new Button("…")` strings under that package. Optional CI scan remains a nice-to-have. |
...
| 6.9 | Replace modal `Alert`-for-validation with inline validation (red border + helper text). Reserve `Alert` for blocking errors. | 29 | **done** — `InlineFieldValidation` helper (`attachError` / `clearError` / `validate`) with CSS pseudo-class wiring; worked-example migrations on `FindStarByCommonNameDialog` + `AddDataSetDialog`. Audit confirmed the remaining ~25 `Alert` sites are legitimately modal preconditions ("select a dataset first", "more than 2 stars required") — not field-validation candidates. |
| 6.10 | Enable Hibernate second-level cache with JCache; choose read-mostly entities; benchmark. | 53 | **done** — JCache + EhCache 3.10 (jakarta classifier) wired as JSR-107 provider via `JCacheBootstrap`; cached singleton prefs + the `SolarSystem` lookup hot path. |
| 6.11a | promptText + Tooltip hints on every input. | 35 | **done** — coverage now on ~45 text inputs across 18 dialogs; every typed-input dialog under `dialogs/*`, `controller/*`, `graphics/*`, `scripting/*`. |
| 6.11b | Responsive layouts (HGrow/VGrow + percentage column constraints). | 34 | **done** — `ResponsiveLayouts` helper landed alongside 9 TableView migrations (one of three resize-policy idioms per table). CLAUDE.md "Responsive Table & Layout Defaults" documents the pattern; per-dialog visual validation at 1024×768 / 4K is the explicit eye-check follow-up (the resize-policy attributes don't validate themselves but no longer prevent reflow). |
...
| 7.3 | try-with-resources sweep across `file/`, `scripting/`, `controller/`, `solarsystem/modelling/accrete/Utils`. | 40, 42 | **done** — the 3 call-out sites fixed in Phase 7.3 (`Utils.loadStarType`, `MainPane` icon load, `ScriptDialog.loadScriptFile`). Bucket-A grep across `service/importservices/` for `new (Buffered|File)(Reader|InputStream|Writer|OutputStream)` returned zero hits outside try-with-resources — that package was already clean. |
...
| 7.7 | Make entity UUID `@Id` fields `final`; add `@PrePersist` assertion. | 45 | **done** — `@PrePersist`/`@PreUpdate` id-non-null assertions added on all three entities (StarObject, SolarSystem, ExoPlanet); ExoPlanet's auto-id `@PrePersist` consolidated with the assertion to avoid undefined-ordering of multiple hooks. `@Id` kept non-final by design: Hibernate hydrates via reflection, so `final` would require unsafe-reflection workarounds — the persist-time assertion catches the actual bug class (silent hash-collection corruption) without that friction. |
| 7.8 | Add `@Basic(fetch = LAZY)` and length caps on `@Lob` columns; benchmark catalog-wide find performance. | 46 | **done** (investigated + reverted; replaced by 7.10/Issue 53). Phase 7.8 added `@Basic(fetch = LAZY)` to 10 `@Lob` fields and wired `hibernate-maven-plugin:7.2.1.Final` for bytecode enhancement, but actual app startup threw `LazyInitializationException` from `PlotManager.drawAstrographicData` (and ~30 other cross-tx readers). All LAZY annotations + the enhancement plugin reverted in close-out. The L2 cache (Issue 53) is the chosen alternative — same "don't re-hit the DB" goal without detached-entity hygiene. |
| 7.9 | Add characterization + happy-path tests to `service/importservices/` (top priority). Then TestFX on the most-used dialogs. Then a smoke test for `SolarSystemSpacePane.setSystemToDisplay`. | 47 | **done** — `PlotStarsCoordinatorTest` (12) + `ImportResultTest` (9) + `ImportTaskControlTest` (4) + `RegCSVFileTest` (3) pinned the pure helpers; closeout adds `CSVDataImportServiceTest` (7) + `CHVDataImportServiceTest` (7) driving each `Service<FileProcessResult>` through real Task lifecycle (succeed / null-result / fail / cancel) and pinning event fan-out + `ImportTaskComplete` callback + `processDataSet` property bindings. Follow-up hardening adds `SolarSystemSpacePaneAsyncLoadTest`, which covers the async `setSystemToDisplay()` path and prevents a regression back to FX-thread DB loading. |
| 7.10 | Build a global `theme.css` with CSS variables. Remove inline `-fx-style` strings. | 50 | **done** — theme.css carries the CSS-variable palette + 25 utility classes (typography, status badges, HZ semantics, temperature deltas, success action button). Stylesheet loaded on the primary Scene by `PrimaryStageInitializer`; standalone programmatic dialogs can now call `DialogUtils.applyTheme(Dialog<?>)`. ~84 inline `setStyle(...)` callsites converted to `getStyleClass().add("trips-…")` across 22 files. Remaining inline styles are dynamic state, genuinely bespoke per-tab colours, or isolated legacy callsites. |
| 7.11 | Accessibility pass: `accessibleText` / `accessibleHelp` on top-used dialogs and controls. | 49 | **done** — accessibility annotations on 35 dialogs (~280 individual calls), every high- and medium-traffic user-facing dialog screen-reader friendly. The final large composite forms, `AddPlanetDialog` and `SpaceshipEditorDialog`, now annotate their dialog panes, primary controls, validation/status surfaces, tables, and action buttons. |
...
| 9.2 | Split `ProceduralPlanetViewerDialog` around a `PlanetViewerViewModel` or `PlanetGenerationSession` before extracting more rendering/regeneration code. | The easy helper extractions are done; the remaining bulk is field/state orchestration and needs a state boundary, not more method shuffling. | done — `PlanetGenerationSession` now owns the generated planet snapshot, derived rainfall/height/plate data references, generation-control parameters, physical terrain-classification context, `PlanetConfig` rebuilding, and availability helpers for overlays/render modes. `ProceduralPlanetViewerDialog` keeps UI controls, scene graph rendering, and callbacks while delegating state orchestration through the session. Added focused `PlanetGenerationSessionTest` coverage for config initialization, control capture/buildConfig, terrain classification, and planet replacement. |
...
- 9.2: `./mvnw-java25.sh -q -pl tripsapplication -Dtest=PlanetGenerationSessionTest test` passed.
- 9.2: `./mvnw-java25.sh -q -pl tripsapplication -DskipTests compile` passed after the `PlanetGenerationSession` extraction.
