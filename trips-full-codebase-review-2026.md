...
- **Status**: partial (Phase 7.10 + close-out + Bucket-A). `theme.css` carries the full CSS-variable palette + utility classes for the most common inline-style patterns: `.trips-bold`, `.trips-text-sm`, `.trips-text-md`, `.trips-text-bold-md`, `.trips-text-muted-sm`, `.trips-text-muted`, `.trips-text-section-header`, plus the existing severity/panel/badge classes. Loaded onto the primary Scene in `PrimaryStageInitializer` so available app-wide. Bucket-A migrated 6 inline `setStyle(...)` callsites in `NebulaEditorDialog` as the worked example (background-color + muted-sm pairs). The remaining ~60 callsites are now a one-line `getStyleClass().add("trips-text-muted-sm")` each — fully mechanical but still a sprint to do exhaustively because each visual replacement should be eyeballed for parity.
