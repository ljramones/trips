...
- **Status**: substantially done (Bucket-A re-audit). `InlineFieldValidation` helper provides `attachError` / `clearError` / `validate` API + CSS pseudo-class wiring; two worked-example migrations in place: `FindStarByCommonNameDialog` (single field) and `AddDataSetDialog` (6 required-field alerts → one shared status label + red-bordered fields). Audit of the remaining ~25 Alert-from-validation sites: **most are NOT field-validation candidates** — they're "select something from the list first" / "no datasets exist yet" / "more than 2 stars required for the route finder" precondition checks where there's no field to highlight, so a modal Alert is the correct tool. The one remaining true field-validation site (`StarEditDialog.changeClicked` catching `NumberFormatException` from `parseDoubleOrThrow`) doesn't know which field threw — fixing it inline would require a per-field validation pass restructure that's bigger work than this Bucket-A scope. Closing at "patterns shipped; remaining alerts are legitimately modal preconditions."
