...
- **Status**: substantially done (Phase 7 close-out + Bucket-A sweep). promptText + Tooltip pairs now on ~30 text inputs across 13 dialogs: the 2 from the close-out (FindStarByCommonNameDialog, FindStarByCatalogIdDialog) plus 11 added in the Bucket-A pass (search dialogs, dataset pickers, exoplanet/gaia import dialogs, routing dialogs, transit filter pane, query dialog). Common example strings: catalog IDs ("e.g. HIP 71683, HD 209458, Gliese 581"), common names ("e.g. Sol, Alpha C, Vega"), distances ("light years"). Most-used input dialogs are covered. ~10 lower-traffic dialogs remain.
...
- **Status**: substantially done (Phase 7.11 + close-out + Bucket-A two-round sweep). Accessibility annotations now on **24 dialogs** (≈170 individual calls): the 4 from Phase 7.11 + 11 from the first Bucket-A round + 13 from the finisher pass: `ViewPreferencesDialog` (preferences), `ExportQueryDialog`, `UpdateStarObjectWithRecordDialog`, `Load10ParsecStarsDialog`, `FindAllByConstellationDialog`, `FindStarInViewDialog`, `ShowStarMatchesDialog`, `SesameNameResolverDialog`, `RouteDialog`, `DataSetDescribeDialog`, `ExportDialog` (dataset), `ComputerInventoryDialog`, `CompareDBDialog`. Every high-traffic and medium-traffic user-facing dialog is now screen-reader friendly. The remaining handful of dialogs are largely low-traffic or read-only displays (e.g. `AboutDialog`).
