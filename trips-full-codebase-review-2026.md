...
- **Status**: done (Phase 7.8 + close-out) — `@Basic(fetch = LAZY)` annotations added to the 6 DataSetDescriptor LOBs, the 3 ExoPlanet procedural LOBs (including the heavy `byte[] proceduralPreview` image), and StarCatalogIds.catalogIdList. `StarObject.notes` + `StarObject.source` intentionally kept eager (broad caller surface). **Bytecode enhancement plugin now wired** so the LAZY annotations actually take effect: `org.hibernate.orm:hibernate-maven-plugin:7.2.1.Final` (the old `org.hibernate.orm.tooling:hibernate-enhance-maven-plugin` was renamed during the Hibernate 7 ASL2 relicense — see [discourse thread](https://discourse.hibernate.org/t/version-7-0-cannot-find-the-hibernate-enhance-maven-plugin-in-maven/11547)). Goal is `enhance`, default phase is `process-classes`. Config sets `enableLazyInitialization=true`; `enableDirtyTracking` and `enableAssociationManagement` stay off because they change observable entity behaviour. Full non-integration suite 2,740 tests green with enhancement on — no detached-entity LIE regressions. Benchmarking the 2M-row dataset win is the remaining-but-optional follow-up.
