...
- **Status**: done. Accessibility annotations now on **33 dialogs** (~200 individual calls): the 4 from Phase 7.11, 11 from the first Bucket-A round, 13 from the second Bucket-A round (`ViewPreferencesDialog`, `ExportQueryDialog`, `UpdateStarObjectWithRecordDialog`, `Load10ParsecStarsDialog`, `FindAllByConstellationDialog`, `FindStarInViewDialog`, `ShowStarMatchesDialog`, `SesameNameResolverDialog`, `RouteDialog`, `DataSetDescribeDialog`, `ExportDialog`, `ComputerInventoryDialog`, `CompareDBDialog`), and 9 from the finisher pass: `AboutDialog`, `ExamplesDialog`, `EachTimeStartDialog`, `RotationDialog`, `StarNotesDialog`, `ScriptDialog`, `RADecToXYZDialog`, `EquatorialToGalacticCoordsDialog`, `NebulaListDialog` (interactive table with 5 action buttons). The remaining unannotated dialogs (`AddPlanetDialog`, `SpaceshipEditorDialog`) are large composite forms whose annotations belong with a focused per-dialog pass, not the mechanical sweep — flagged for any future dedicated accessibility design pass.
...
- **Status**: done (Phase 7.10 + close-out + 4 Bucket-A passes). theme.css carries the CSS-variable palette + 25 utility classes: the original 7 (.trips-bold, .trips-text-sm/md, .trips-text-bold-md, .trips-text-muted-sm, .trips-text-muted, .trips-text-section-header, .trips-panel-mid), 5 second-pass classes (.trips-text-bold-md-12, .trips-text-bold-lg, .trips-text-bold-sm, .trips-text-italic-muted-sm, .trips-text-italic-warn), and 13 closeout classes (.trips-text-form-label, .trips-text-form-label-sm, .trips-text-form-info, .trips-text-hz-optimal, .trips-text-hz-max, .trips-text-cooling, .trips-text-warming, .trips-button-action-success, .trips-badge-true, .trips-badge-false, .trips-badge-hz-optimal, .trips-badge-hz-extended, .trips-badge-hz-outside). Stylesheet loaded on the primary Scene by PrimaryStageInitializer. Across all migration passes: ~84 callsites converted to `getStyleClass().add("trips-…")` across 22 files; the closeout pass alone replaced 47 inline `setStyle(...)` strings in `ProceduralPlanetViewerDialog` (29), `PlanetTab` (13), and `PlanetDialog` (5). Repo-wide inline `setStyle(` count went from 102 → 56. The remaining 56 are dynamic per-instance state (cursor hover/normal/highlight constants), genuinely bespoke per-tab pastels in `ViewPreferencesDialog`, or the `InlineFieldValidation` fallback (Javadoc-flagged as temporary until theme.css is wired into every dialog scene — a separate refactor that's not part of #50's scope).
...
| 7.9 | Add characterization + happy-path tests to `service/importservices/` (top priority). Then TestFX on the most-used dialogs. Then a smoke test for `SolarSystemSpacePane.setSystemToDisplay`. | 47 | **done** — `PlotStarsCoordinatorTest` (12) + `ImportResultTest` (9) + `ImportTaskControlTest` (4) + `RegCSVFileTest` (3) pinned the pure helpers; closeout adds `CSVDataImportServiceTest` (7) + `CHVDataImportServiceTest` (7) driving each `Service<FileProcessResult>` through real Task lifecycle (succeed / null-result / fail / cancel) and pinning event fan-out + `ImportTaskComplete` callback + `processDataSet` property bindings. The `SolarSystemSpacePane.setSystemToDisplay()` smoke test genuinely needs TestFX scaffolding (scene-graph-heavy constructor + render-via-Task) — separate dependency-introduction decision, flagged as the explicit future follow-up. |
| 7.10 | Build a global `theme.css` with CSS variables. Remove inline `-fx-style` strings. | 50 | **done** — theme.css carries the CSS-variable palette + 25 utility classes (typography, status badges, HZ semantics, temperature deltas, success action button). Stylesheet loaded on the primary Scene by `PrimaryStageInitializer`. ~84 inline `setStyle(...)` callsites converted to `getStyleClass().add("trips-…")` across 22 files. Repo-wide inline `setStyle(` count: 102 → 56. Remaining 56 are dynamic state, genuinely bespoke per-tab pastels, or the `InlineFieldValidation` fallback (Javadoc-flagged as temporary until theme.css is loaded on every dialog scene — separate refactor). |
| 7.11 | Accessibility pass: `accessibleText` / `accessibleHelp` on top-used dialogs and controls. | 49 | **done** — accessibility annotations on 33 dialogs (~200 individual calls), every high- and medium-traffic user-facing dialog screen-reader friendly. Only large composite forms (`AddPlanetDialog`, `SpaceshipEditorDialog`) remain unannotated — flagged for any future dedicated accessibility design pass. |
