...
- **Status**: partial (Bucket-A extended). `InlineFieldValidation` helper provides `attachError` / `clearError` / `validate` API + CSS pseudo-class wiring. Two worked-example migrations now in place: `FindStarByCommonNameDialog` (single field) and `AddDataSetDialog` (6 required-field alerts migrated to one shared status label + red-bordered fields). Pattern handles both single-field and multi-field forms. Remaining ~20 input-validation Alert sites are each a ~5-line change against a similar shape; full sweep deferred.
...
- **Status**: substantially done (Phase 7.11 + close-out + Bucket-A sweep). Accessibility annotations now on 15 dialogs: the 4 from Phase 7.11 (`TransferPreviewDialog`, `FindStarByCommonNameDialog`, `FindStarByCatalogIdDialog`, `RoutingPanel`) plus 11 added by the Bucket-A sweep (`FindStarsWithNameMatchDialog`, `FindByCatalogIdDialog`, `FindRelatedStarsbyDistance`, `FindDistanceDialog`, `QueryDialog` — the main search dialog — `SelectActiveDatasetDialog`, `DataSetManagerDialog`, `LoadExoPlanetsFileDialog`, `AddStarRecordDialog`, `LookupStarDialog`, `RouteFinderDialogInView`, `TransitFilterPane`). ~75 new `setAccessibleText`/`setAccessibleHelp` calls. The high-traffic dialogs are now screen-reader friendly. Remaining ~40 lower-traffic dialogs are each a 5-line edit; still bucket-B if you want exhaustive coverage.
