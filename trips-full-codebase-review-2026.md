...
- **Status**: substantially done (Phase 7.10 + close-out + 3 Bucket-A passes). theme.css carries the CSS-variable palette + 13 utility classes: the original 7 (.trips-bold, .trips-text-sm/md, .trips-text-bold-md, .trips-text-muted-sm, .trips-text-muted, .trips-text-section-header, .trips-panel-mid) plus 5 second-pass classes (.trips-text-bold-md-12, .trips-text-bold-lg, .trips-text-bold-sm, .trips-text-italic-muted-sm, .trips-text-italic-warn). Stylesheet loaded on the primary Scene by PrimaryStageInitializer. Across all migration passes: ~37 callsites converted to `getStyleClass().add("trips-…")` across 19 files. Remaining ~25 callsites use custom colours (#888, #555, #333, gray, semi-transparent overlays) or multi-property layout combos (-fx-padding + -fx-background-radius + -fx-border-*) that need bespoke design rather than a utility class.
