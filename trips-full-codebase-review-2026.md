...
| 4.7 | `SolarSystemService` **793 → 669 lines** (-16%). `GeneratedSystemPersister` (227 lines) extracted as a new `@Component`: owns the three ACCRETE save paths — `savePlanets` (planets + moons + simulated-row cleanup + planet-count + habitable-zone update), `saveBelts` (asteroid + Kuiper `SolarSystemFeature` rows), and the `saveSystem` convenience that chains both. Cross-service dependency on `findOrCreateSolarSystem` is passed in as a `Function<StarObject, SolarSystem>` so the persister stays decoupled. `SolarSystemService` keeps the public `saveGenerated*` methods as thin delegates (`this::findOrCreateSolarSystem`) so existing callers (e.g. `StarContextMenuHandler.java:303`) don't change. Belt-builder bodies factored into private static `buildAsteroidBelt` / `buildKuiperBelt` inside the persister. Two stale `{:.2f}` log placeholders fixed in passing (used SLF4J but with Python-style format spec). `PlanetarySkyRenderer` (1,204) and `MainSplitPaneManager` (670) remain deferred. Full non-integration suite 2,718 tests green. | 9 (followup), 18 | **partial** |
