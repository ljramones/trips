...
- **Status**: pattern shipped (Phase 6.4 + Bucket-A re-audit). `TransferPlannerPanel.configureNodeTable` is the worked example. Bucket-A grep found ~24 files with hardcoded `setPrefWidth(...)` values across nebula/dataset/transit/workbench/procedural dialogs. Blind per-file migration is unsafe — each conversion (HGrow.ALWAYS + percentage column constraints) needs a visual check at both 1024×768 and 4K to confirm no truncation or stretch artefact, which is UX work not mechanical work. The pattern is documented + ready to apply when someone does the per-dialog UX validation pass.
...
| 6.11b | Responsive layouts (HGrow/VGrow + percentage column constraints). | 34 | **partial** — pattern shipped in `TransferPlannerPanel.configureNodeTable`. Bucket-A grep found ~24 files with hardcoded `setPrefWidth` values; per-dialog conversion needs visual validation at 1024×768 and 4K, which is UX work rather than mechanical sweep. |
