...
- **Subscribers** (1 site):
  - `tripsapplication/src/main/java/com/teamgannon/trips/controller/MainPane.java:298` in `onOpenWorkbenchEvent`
...
None as of Phase 9.6. The convention test keeps the catalog headings aligned
with the current `*Event.java` sources so deleted event entries do not linger.
