...
import com.teamgannon.trips.planetarymodelling.planetgen.generator.PlanetGenerator;
...
import com.teamgannon.trips.planetarymodelling.planetgen.planet.*;
import com.teamgannon.trips.planetarymodelling.planetgen.planet.layer.CraterLayer;
import com.teamgannon.trips.planetarymodelling.planetgen.planet.layer.PlanetPoint;
import com.teamgannon.trips.planetarymodelling.planetgen.planet.layer.PlantLayer;
...
import com.teamgannon.trips.planetarymodelling.util.Tuple2;
...
public class PlanetGeneratorJavafxApp extends Application {
	...
	private final LongProperty seedProperty = new SimpleLongProperty();
	private final LongProperty timeProperty = new SimpleLongProperty();
	private final LongProperty revolutionTimeProperty = new SimpleLongProperty();
	private final LongProperty orbitTimeProperty = new SimpleLongProperty();
	private final DoubleProperty radiusProperty = new SimpleDoubleProperty();
	private final DoubleProperty minHeightProperty = new SimpleDoubleProperty();
	private final DoubleProperty maxHeightProperty = new SimpleDoubleProperty();
	private final BooleanProperty hasOceanProperty = new SimpleBooleanProperty();
	private final DoubleProperty craterDensityProperty = new SimpleDoubleProperty();
	private final DoubleProperty volcanoDensityProperty = new SimpleDoubleProperty();
	...
	private final DoubleProperty dailyTemperatureGroundDelayProperty = new SimpleDoubleProperty();
	private final DoubleProperty dailyTemperatureOceanFactorProperty = new SimpleDoubleProperty();
	...
	private Node createCratersInfoView() {
        ...
        cratersListView.getSelectionModel().selectedItemProperty().addListener((observable, oldCrater, newCrater) -> {
    		...
    		for (double x = -1.1; x <= 1.1; x+=0.005) {
				...
				heightData.add(new Data<>(x, height));
    			heightNoiseData.add(new Data<>(x, newCrater.verticalHeightNoiseFunction.calculate(x)));
