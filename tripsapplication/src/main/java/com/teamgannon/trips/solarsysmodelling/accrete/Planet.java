...
//                    java.lang.System.out.println("Found chemical " + chems[i].symbol + " with a yp = " + yp + " a low temp of " + this.lowTemperature + "  and a min molar mass of " + this.minimumMolecularWeight + " with amount " + amount[i]);
    ...
    public String datasheet(String prepend) {
        ...
        retval.append(cr).append(prepend)
                .append("  Mass: ")
                .append(this.gasGiant ? "%1$,.2f".formatted(massInJupiterMasses()) + " jm, " : "")
                .append("%1$,.2f".formatted(massInEarthMasses()))
                .append(" em, ")
                .append("%.3E".formatted(massInKg()))
                .append(" kg, density - ")
                .append("%1$,.2f".formatted(this.density))
                .append(" g/cc, radius - ")
                .append("%1$,.2f".formatted(radiusInMeters() / 1000.0))
                .append("km");
        if (this.isMoon) {
            retval.append(cr)
                    .append(prepend)
                    .append("  Orbit: SMA - ")
                    .append("%.3E".formatted(AUtoKm(this.asMoonSMA)))
                    .append(" km, eccentricity - ")
                    .append("%1$,.3f".formatted(this.asMoonEccentricty))
                    .append(", apoapsis: ")
                    .append("%.3E".formatted(AUtoKm(asMoonApoapsis())))
                    .append(" km, periapsis: ")
                    .append("%.3E".formatted(AUtoKm(asMoonPeriapsis())))
                    .append(" km");
        } else {
            retval.append(cr)
                    .append(prepend)
                    .append("  Orbit: SMA - ")
                    .append("%1$,.2f".formatted(this.sma))
                    .append(" AU, eccentricity - ")
                    .append("%1$,.3f".formatted(this.eccentricity))
                    .append(", apoapsis: ")
                    .append("%1$,.4f".formatted(apoapsis()))
                    .append(" AU, periapsis: ")
                    .append("%1$,.2f".formatted(periapsis()))
                    .append(" AU")
                    .append(" [")
        ...
        retval.append(cr)
                .append(prepend)
                .append("  Axial Tilt: ")
                .append("%1$,.2f".formatted(this.axialTilt))
                .append(", Day: ")
                .append("%1$,.2f".formatted(secondsToHours(this.dayLength)))
                .append(" hours, Year: ")
                .append("%1$,.2f".formatted(secondsToYears(this.orbitalPeriod)))
                .append(" Earth years");
        if (!this.gasGiant) {
            retval.append(cr).append(prepend)
                    .append("  Surface: Gravity - ")
                    .append("%1$,.2f".formatted(this.surfaceGravity))
                    .append(", Temperature - ")
                    .append("%1$,.2f".formatted(this.surfaceTemperature - 273.15))
                    .append("C (")
                    .append("%1$,.2f".formatted(this.surfaceTemperature * (9.0 / 5.0) - 459.67))
                    .append("F)");
            retval.append(cr)
                    .append(prepend).append("  Water: ")
                    .append("%1$,.2f".formatted(this.hydrosphere * 100.0))
                    .append("%, Cloud Cover: ")
                    .append("%1$,.2f".formatted(this.cloudCover * 100.0))
                    .append("%, Ice Cover: ")
                    .append("%1$,.2f".formatted(this.iceCover * 100.0))
                    .append("%");
            retval.append(cr)
                    .append(prepend)
                    .append("  Min Temperature - ")
                    .append("%1$,.2f".formatted(this.minTemperature - 273.15))
                    .append("C (")
                    .append("%1$,.2f".formatted(this.minTemperature * (9.0 / 5.0) - 459.67))
                    .append("F), Low Temperature - ")
                    .append("%1$,.2f".formatted(this.lowTemperature - 273.15))
                    .append("C (")
                    .append("%1$,.2f".formatted(this.lowTemperature * (9.0 / 5.0) - 459.67))
                    .append("F), High Temperature - ")
                    .append("%1$,.2f".formatted(this.highTemperature - 273.15))
                    .append("C (")
                    .append("%1$,.2f".formatted(this.highTemperature * (9.0 / 5.0) - 459.67))
                    .append("F), Max Temperature - ")
                    .append("%1$,.2f".formatted(this.maxTemperature - 273.15))
                    .append("C (")
                    .append("%1$,.2f".formatted(this.maxTemperature * (9.0 / 5.0) - 459.67))
                    .append("F)");
            double yp = (373.0 * ((log(this.surfacePressure / 1000.0 + 0.001) / -5050.5) + (1.0 / 373.0)));
            retval.append(cr)
                    .append(prepend)
                    .append("  Atmosphere: Pressure - ")
                    .append("%1$,.2f".formatted(this.surfacePressure / 1000.0))
                    .append(" bar, Minimum Molecular Weight - ")
                    .append("%1$,.3f".formatted(this.minimumMolecularWeight))
                    .append(", yp - ")
                    .append("%1$,.3f".formatted(yp));
        }
