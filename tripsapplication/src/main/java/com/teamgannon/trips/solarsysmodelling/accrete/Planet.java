...
*/
    ...
    public void finalize(boolean doMoons) {
        ...
        if (this.type == PlanetTypeEnum.tGasGiant || this.type == PlanetTypeEnum.tSubGasGiant || this.type == PlanetTypeEnum.tSubSubGasGiant) {
        ...
        if (this.type != PlanetTypeEnum.tSubSubGasGiant && this.type != PlanetTypeEnum.tSubGasGiant && this.type != PlanetTypeEnum.tGasGiant) {
            this.gasGiant = false;
        } else {
            this.gasGiant = true;
        ...
        if (breathableAtmosphere() == AtmosphereTypeEnum.BREATHABLE) {
            if (!(this.resonantPeriod || secondsToHoursRounded(this.dayLength) == secondsToHoursRounded(this.orbitalPeriod))) {
                ...
                if (this.surfaceGravity >= 0.8 && this.surfaceGravity <= 1.2 && relTemp >= -2.0 && relTemp <= 3.0 && this.iceCover <= 0.1 && pressure >= 0.5 && pressure <= 2.0 && this.cloudCover >= 0.4 && this.cloudCover <= 0.8 && this.hydrosphere >= 0.5 && this.hydrosphere <= 0.8 && this.type != PlanetTypeEnum.tWater) {
                    this.earthlike = true;
                }
            }
        }
    ...
    public void calculateGases() {
        if (this.surfacePressure > 0.0) {
            ...
            for (int i = 0; i < chems.length; i++) {
                double yp = chems[i].getBoilingPoint() / (373.0 * ((Math.log(pressure + 0.001) / -5050.5) + (1.0 / 373.0)));

                if ((yp >= 0.0 && yp < this.lowTemperature) && chems[i].getWeight() >= this.minimumMolecularWeight) {
                    ...
                    if (chems[i].getAtomicNumber() == 18) {
                        ...
                        pres2 = (0.75 + pressure);
                        react = Math.pow(1.0 / (1.0 + chems[i].getReactivity()), this.primary.age / 2e9 * pres2);
                    } else if ((chems[i].getAtomicNumber() == 8 || chems[i].getAtomicNumber() == 912) && this.primary.age > 2e9 && this.surfaceTemperature > 270.0 && this.surfaceTemperature < 400.0) {
                        /*	pres2 = (0.65 + pressure/2); Breathable - M: .55-1.4 */
                        pres2 = (0.89 + pressure / 4); /*	Breathable - M: .6 -1.8 */
                        react = Math.pow(1.0 / (1.0 + chems[i].getReactivity()), Math.pow(this.primary.age / 2e9, 0.25) * pres2);
                    } else if (chems[i].getAtomicNumber() == 902 && this.primary.age > 2e9 && this.surfaceTemperature > 270 && this.surfaceTemperature < 400) {
                        pres2 = (0.75 + pressure);
                        react = Math.pow(1.0 / (1.0 + chems[i].getReactivity()), Math.pow(this.primary.age / 2e9, 0.5) * pres2);
                        react *= 1.5;
                    } else {
                        pres2 = (0.75 + pressure);
                        react = Math.pow(1.0 / (1.0 + chems[i].getReactivity()), this.primary.age / 2e9 * pres2);
    ...
    public String datasheet(String prepend) {
        ...
        if (!this.gasGiant) {
            if (this.habitable) {
                ...
                retval.append(" (").append(atmosphereType()).append(")");
            }
        }
        if (this.resonantPeriod) {
            retval.append(" - tidally locked or resonant period.");
        }
        retval.append(cr).append(prepend).append("  Mass: ").append(this.gasGiant ? String.format("%1$,.2f", massInJupiterMasses()) + " jm, " : "").append(String.format("%1$,.2f", massInEarthMasses())).append(" em, ").append(String.format("%.3E", massInKg())).append(" kg, density - ").append(String.format("%1$,.2f", this.density)).append(" g/cc, radius - ").append(String.format("%1$,.2f", radiusInMeters() / 1000.0)).append("km");
        if (this.isMoon) {
            retval.append(cr).append(prepend).append("  Orbit: SMA - ").append(String.format("%.3E", AUtoKm(this.asMoonSMA))).append(" km, eccentricity - ").append(String.format("%1$,.3f", this.asMoonEccentricty)).append(", apoapsis: ").append(String.format("%.3E", AUtoKm(asMoonApoapsis()))).append(" km, periapsis: ").append(String.format("%.3E", AUtoKm(asMoonPeriapsis()))).append(" km");
        } else {
            retval.append(cr).append(prepend).append("  Orbit: SMA - ").append(String.format("%1$,.2f", this.sma)).append(" AU, eccentricity - ").append(String.format("%1$,.3f", this.eccentricity)).append(", apoapsis: ").append(String.format("%1$,.4f", apoapsis())).append(" AU, periapsis: ").append(String.format("%1$,.2f", periapsis())).append(" AU");
        }
        // TODO: Fix the following values for moons so that they correctly use their moon SMA and eccentricity
        retval.append(cr).append(prepend).append("  Axial Tilt: ").append(String.format("%1$,.2f", this.axialTilt)).append(", Day: ").append(String.format("%1$,.2f", secondsToHours(this.dayLength))).append(" hours, Year: ").append(String.format("%1$,.2f", secondsToYears(this.orbitalPeriod))).append(" Earth years");
        if (!this.gasGiant) {
            retval.append(cr).append(prepend).append("  Surface: Gravity - ").append(String.format("%1$,.2f", this.surfaceGravity)).append(", Temperature - ").append(String.format("%1$,.2f", this.surfaceTemperature - 273.15)).append("C (").append(String.format("%1$,.2f", this.surfaceTemperature * (9.0 / 5.0) - 459.67)).append("F)");
            retval.append(cr).append(prepend).append("  Water: ").append(String.format("%1$,.2f", this.hydrosphere * 100.0)).append("%, Cloud Cover: ").append(String.format("%1$,.2f", this.cloudCover * 100.0)).append("%, Ice Cover: ").append(String.format("%1$,.2f", this.iceCover * 100.0)).append("%");
            retval.append(cr).append(prepend).append("  Min Temperature - ").append(String.format("%1$,.2f", this.minTemperature - 273.15)).append("C (").append(String.format("%1$,.2f", this.minTemperature * (9.0 / 5.0) - 459.67)).append("F), Low Temperature - ").append(String.format("%1$,.2f", this.lowTemperature - 273.15)).append("C (").append(String.format("%1$,.2f", this.lowTemperature * (9.0 / 5.0) - 459.67)).append("F), High Temperature - ").append(String.format("%1$,.2f", this.highTemperature - 273.15)).append("C (").append(String.format("%1$,.2f", this.highTemperature * (9.0 / 5.0) - 459.67)).append("F), Max Temperature - ").append(String.format("%1$,.2f", this.maxTemperature - 273.15)).append("C (").append(String.format("%1$,.2f", this.maxTemperature * (9.0 / 5.0) - 459.67)).append("F)");
            double yp = (373.0 * ((Math.log(this.surfacePressure / 1000.0 + 0.001) / -5050.5) + (1.0 / 373.0)));
            retval.append(cr).append(prepend).append("  Atmosphere: Pressure - ").append(String.format("%1$,.2f", this.surfacePressure / 1000.0)).append(" bar, Minimum Molecular Weight - ").append(String.format("%1$,.3f", this.minimumMolecularWeight)).append(", yp - ").append(String.format("%1$,.3f", yp));
        }
        if (this.atmosphere != null) {
            if (this.atmosphere.size() > 0) {
                retval.append(cr).append(prepend).append("  Atmospheric Constituents:");
                AtmosphericChemical ac;
                for (AtmosphericChemical atmosphericChemical : atmosphere) {
                    ac = atmosphericChemical;
                    retval.append(cr).append(prepend).append("    ").append(ac.getChem().getSymbol()).append(" (").append(ac.getChem().getName()).append(") ").append(String.format("%1$,.4f", ac.getSurfacePressure() / 1000.0)).append(" bar");
                }
            }
        }
