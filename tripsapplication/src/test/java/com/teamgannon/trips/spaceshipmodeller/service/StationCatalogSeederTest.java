...
/**
 * Unit tests for {@link StationCatalogSeeder}. The sync-by-id logic lives on the service (and is
 * covered there); this class confirms that the listener delegates exactly once per application-
 * ready event and survives a service-side exception without propagating it.
 */
...
class StationCatalogSeederTest {
    ...
    void readyEventDelegatesOnce() {
        when(service.syncCatalogEntries()).thenReturn(1);
