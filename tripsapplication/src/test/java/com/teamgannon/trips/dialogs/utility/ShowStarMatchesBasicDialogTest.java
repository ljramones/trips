...
class ShowStarMatchesBasicDialogTest {

    private ShowStarMatchesBasicDialog dialog;
    ...
    void testDialogCreation() throws Exception {
        ...
        Platform.runLater(() -> {
            try {
                dialog = new ShowStarMatchesBasicDialog(testStars);
                assertNotNull(dialog, "Dialog should be created");
                assertEquals("Show discovered stars", dialog.getTitle());
            } finally {
                latch.countDown();
            }
        });
    ...
    void testTableHasCorrectColumns() throws Exception {
        ...
        Platform.runLater(() -> {
            try {
                ...
                assertTrue(columnNames.contains("Catalog Id List"), "Should have Catalog Id List column");
    ...
    @Test
    void testContextMenuExists() throws Exception {
        CountDownLatch latch = new CountDownLatch(1);
        ...
        Platform.runLater(() -> {
            try {
                ...
                TableView<StarObject> tableView = findTableView();
                assertNotNull(tableView, "TableView should exist");
                assertNotNull(tableView.getContextMenu(), "TableView should have context menu");

                // Check that context menu has the "Choose" item
                boolean hasChooseItem = tableView.getContextMenu().getItems().stream()
                        .anyMatch(item -> "Choose".equals(item.getText()));
                assertTrue(hasChooseItem, "Context menu should have 'Choose' item");
            } finally {
                latch.countDown();
            }
        });
